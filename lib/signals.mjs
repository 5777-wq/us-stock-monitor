...
/* 汇总：分组输出（SPMO 单列 + 超买 + 超卖 + 通道事件 + 平仓事件） */
export function summarize(rows) {
  ...
  const events = ok.filter((r) =>
    (r.rsi6 && r.rsi6.cross && r.rsi6.cross !== 'into_ob' && r.rsi6.cross !== 'into_os') ||
    r.tunnels.some((t) => t.event));
  const newInto = ok.filter((r) => r.rsi6 && (r.rsi6.cross === 'into_ob' || r.rsi6.cross === 'into_os'));
  return { all: ok, overbought: ob, oversold: os, events, newInto };
}
