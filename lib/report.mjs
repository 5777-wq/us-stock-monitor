...
export function consoleReport(data) {
  ...
  L.push(`数据：东财后复权 ${meta.srcEast} 只 · 腾讯不复权兜底 ${meta.srcTx} 只 · 失败 ${meta.srcFail} 只${meta.eastError ? ` · 东财: ${meta.eastError}` : ''}`);
  L.push('═'.repeat(96));
  ...
  section('今日事件', summary.events, 'RSI 穿越回落 / 通道上下穿');
...
function jsonPayload(data) {
  ...
  const slim = (r) => r && ({
    ...
    rsi6: r.rsi6, tunnels: r.tunnels, live: r.live, adj: r.adj, source: r.source, bars: r.bars, notes: r.notes,
  });
  return {
    version: 1,
...
export function htmlPayload(data) {
  ...
  const pageRows = data.rows.filter(Boolean).map((r) => ({ ...r, spark: r.spark || null }));
  const json = JSON.stringify({ meta, cfg, spmo: data.spmo, rows: pageRows, groups: { overbought: summary.overbought.map((r) => r.ticker), oversold: summary.oversold.map((r) => r.ticker), events: summary.events.map((r) => r.ticker) } })
    .replace(/</g, '\\u003c');
  return `<!DOCTYPE html>
...
/* chips */
const groups={all:'全部('+([D.spmo].filter(Boolean).length+D.rows.length)+')',ob:'超买('+D.groups.overbought.length+')',os:'超卖('+D.groups.oversold.length+')',ev:'今日事件('+D.groups.events.length+')',spmo:'SPMO重合'};
let cur='all';
...
/* table */
function inGroup(r){if(cur==='all')return true;if(cur==='spmo')return r.spmoPct!=null;if(!byT[r.ticker])return false;return cur==='ob'?D.groups.overbought.includes(r.ticker):cur==='os'?D.groups.oversold.includes(r.ticker):D.groups.events.includes(r.ticker)}
...
/* detail */
function svg(r){const c=r.spark;if(!c||!c.closes||!c.closes.length)return '';const N=c.closes.length;const W=880,H=300,RH=70,pad=6;
const all=[];const series=[{v:c.closes,col:'var(--fg)',wd:1.4}];const ns=[[12,36,'#8e7cc3','12/36'],[144,169,'#e69138','144/169'],[576,676,'#3d85c6','576/676']];
ns.forEach(([a,b,col,lb])=>{const ea=[],eb=[];for(let i=0;i<N;i++){ea.push(emaAt(c.closes.slice(0,i+1),a));eb.push(emaAt(c.closes.slice(0,i+1),b))}series.push({v:ea,col,wd:1},{v:eb,col,wd:1})});
const rr=rsiSeries(c.closes,6);
series.forEach(s=>s.v.forEach(x=>{if(x!=null)all.push(x)}));rr.forEach(x=>{if(x!=null)all.push(x)});
const lo=Math.min(...all),hi=Math.max(...all),span=(hi-lo)||1;
const X=i=>pad+(W-2*pad)*i/(N-1),Y=v=>pad+(H-RH-2*pad)*(1-(v-lo)/span);
let out='<svg viewBox="0 0 '+W+' '+H+'" xmlns="http://www.w3.org/2000/svg">';
series.forEach((s,si)=>{let d='',pen=false;for(let i=0;i<N;i++){if(s.v[i]==null){pen=false;continue}d+=(pen?'L':'M')+X(i).toFixed(1)+' '+Y(s.v[i]).toFixed(1)+' ';pen=true}if(d)out+='<path d="'+d+'" fill="none" stroke="'+s.col+'" stroke-width="'+s.wd+'" '+(si===0?'':'stroke-dasharray="3 2"')+'/>'});
out+='<text x="'+(W-8)+'" y="'+(pad+12)+'" text-anchor="end" font-size="11" fill="var(--mut)">收盘 · 12/36 紫 · 144/169 橙(Vegas) · 576/676 蓝</text>';
const y2=H-pad,Rtop=H-RH;const YR=v=>Rtop+(RH-2*pad)*(1-v/100);
let d2='';for(let i=0;i<N;i++){if(rr[i]==null)continue;d2+=(d2?'L':'M')+X(i).toFixed(1)+' '+YR(rr[i]).toFixed(1)+' '}
out+='<path d="M'+pad+' '+(Rtop-pad)+'L'+(W-pad)+' '+(Rtop-pad)+'L'+(W-pad)+' '+y2+'L'+pad+' '+y2+'Z" fill="var(--chipbg)" stroke="var(--line)"/>';
[70,30].forEach(g=>{out+='<line x1="'+pad+'" y1="'+YR(g)+'" x2="'+(W-pad)+'" y2="'+YR(g)+'" stroke="var(--line)" stroke-dasharray="4 3"/><text x="'+(W-pad+2)+'" y="'+(YR(g)+3)+'" font-size="10" fill="var(--mut)">'+g+'</text>'});
if(d2)out+='<path d="'+d2+'" fill="none" stroke="var(--hot)" stroke-width="1.4"/>';
out+='<text x="'+pad+'" y="'+(Rtop+14)+'" font-size="11" fill="var(--hot)">RSI6（红）70/30 虚线</text></svg>';return out}
function show(tk){const r=byT[tk];if(!r)return;const p=document.getElementById('panel');
p.innerHTML='<span class="close-x" id="detail-close">×</span><h3>'+esc(r.ticker)+' · '+esc(r.name)+(r.rank?' <span class="mut">市值排名 #'+r.rank+'</span>':'')+'</h3><div class="meta">截至 '+esc(r.asOf)+' 收盘 · 收盘 '+num(r.close)+' ('+pct(r.chgPct)+') · RSI6 '+num(r.rsi6?r.rsi6.value:null,1)+' · 数据源 '+esc(r.source)+' · '+(r.adj==='hfq'?'后复权(含拆股分红)':'不复权(拆股会使长通道失真)')+' · '+r.bars+' 根'+(r.spmoPct!=null?' · SPMO持仓 '+r.spmoPct.toFixed(2)+'%':'')+'</div>'+svg(r)+'<div class="notes">'+(r.notes||[]).map(n=>'<div>['+esc(n.tag)+'] '+esc(n.text)+'</div>').join('')+'</div>';
