...
/* detail */
function svg(r){const c=r.spark;if(!c||!c.closes||!c.closes.length)return '';const N=c.closes.length;const W=880,H=300,RH=70,pad=6,BM=18;
...
let out='<svg viewBox="0 0 '+W+' '+(H+BM)+'" xmlns="http://www.w3.org/2000/svg">';
...
[70,30].forEach(g=>{out+='<line x1="'+pad+'" y1="'+YR(g)+'" x2="'+(W-pad)+'" y2="'+YR(g)+'" stroke="var(--line)" stroke-dasharray="4 3"/><text x="'+(W-pad-4)+'" y="'+(YR(g)-3)+'" text-anchor="end" font-size="10" fill="var(--mut)">'+g+'</text>'});
if(d2)out+='<path d="'+d2+'" fill="none" stroke="var(--hot)" stroke-width="1.4"/>';
out+='<text x="'+pad+'" y="'+(H+BM-5)+'" font-size="11" fill="var(--hot)">RSI6（红）· 70/30 虚线为超买/超卖参考</text></svg>';return out}
