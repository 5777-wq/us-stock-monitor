...
export async function buildUniverse({ topN, candidatesFile, quoteBatch }) {
  ...
  const quotes = await tencentQuotes(candidates.tickers.map((t) => t.ticker), { batch: quoteBatch || 60 });
  ...
  return { universe, candidatesMeta: candidates.meta, spmoHoldings: spmo };
}
